package com.doctor330.cloud.utils;
...
public class ClassUtils {
	...
	/**
	 * 根据文件所在类路径返回对应的绝对路径的文件对象。 该文件的URL是类装载器在类路径上找到的第一个该路径上的文件的URL
	 * 
	 * @param classPath
	 *            文件类路径
	...
	public static File getClassPathFile(String classPath)
		...
		if (url != null && "file".equals(url.getProtocol()))
			...
			throw new FileNotFoundException("this file [" + url
					+ "] in class path:[" + classPath
					+ "] not a filesystem file");
		else
			throw new FileNotFoundException("this file not in class path:["
					+ classPath + "]");
	}

	/**
	 * 根据文件所在类路径返回该文件对应的绝对URLs。 该URL数组是类装载器在类路径上找到的所有该路径文件的URL的集合
	...
	public static URL[] getClassPathFileURLs(String classPath)
			throws IOException {
		...
		while (e.hasMoreElements()) {
			...
			set.add((URL) o);
			log.debug("find file [" + classPath
					+ "] from class path, and url is [" + o + "]");
		}
		return set.toArray(new URL[0]);
	}

	/**
	 * 根据文件所在类路径返回该文件对应的绝对URL。 该URL是类装载器在类路径上找到的第一个该路径上的文件的URL
	 * URL格式可能是file:/D:/foo/bar/classes/com/guanghua/brick/aaa.class
	 * 也有可能是jar:file:/D:/foo/bar/brick.jar!com/guanghua/brick/aaa.class等等
	 * 
	 * @param classPath
	 ...
	 * 搜索到一个文件是c
	 * :/foo/bar/test/Test.class,那么返回的list中的字符串就是foo1/bar1/test/Test.class
	 * 如果prefix传的是"",那么返回的字符串就是/test/Test.class
	 * 
	 * @param folder
	 *            , 开始搜索的文件夹，如果文件夹为空，或者不存在，或者不是文件夹，那么返回空列表
	 * @param pattern
	 *            , 搜索用的正则表达式
	 * @param prefix
	 *            , 返回类路径或者是文件绝对路径的标志，如果是null，返回文件绝对路径
			...
			for (int i = 0; files != null && i < files.length; i++) {
				if (files[i].isDirectory()) {
					re.addAll(searchFileFromFolder(files[i], pattern,
							prefix == null ? null : prefix + files[i].getName()));
				} else {
					// 需要返回文件路径
					if (prefix == null)
	...
	public static List<Field> getAllFields(Class<?> c) {
		...
		CollectionUtils.addAll(fields, c.getDeclaredFields());
		
		getParentFields(c, fields);
		
		return fields;
	}

	public static List<Field> getParentFields(Class<?> c, List<Field> fields) {
		...
		CollectionUtils.addAll(fields, superClass.getDeclaredFields());
		
		List<Field> parentFields = getParentFields(superClass, fields);
